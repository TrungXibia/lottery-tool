...
def serve_static(path):
    # This is a fallback, but index.html should handle most static assets.
...
def _clean_df(df: pd.DataFrame):
    ...
    for col in df.columns:
        df[col] = df[col].map(lambda v: format_result(v, col))
...
def api_run_analysis():
    ...
    is_year_data, row_idx, num_patterns = data.get('is_year_data', False), int(data.get('day_idx', -1)), int(data.get('num_patterns', 2))
    ...
    if is_year_data:
        ...
        if num_patterns >= 2:
            try:
                ...
                else: temp_patterns.append('')
            except (ValueError, IndexError): temp_patterns.append('')
        if num_patterns >= 1:
            if row_idx > 0:
                pat_val = df.iloc[row_idx - 1][selected_month_col]
                temp_patterns.append(pat_val[-2:] if isinstance(pat_val, str) and len(pat_val) >= 2 else '')
                pattern_months.add(selected_month_col)
            else: temp_patterns.append('')
    ...
    match_func = lambda v, p: isinstance(v, str) and len(v) >= 2 and v[-2:] == p
    ...
    cols_to_scan = [c for c in cols_full if c != 'Ngày' and c not in cols_to_skip]
    ...
    for dir_idx, inside in enumerate([True, False]):
        for step in range(6):
            ...
            for col_name in cols_to_scan:
                ...
                for i in range(len(df)):
                    ...
                    for k in range(num_patterns):
                        ...
                        if not match_func(df.iloc[row_pos][col_name], patterns[k]):
                            ok = False
                            break
            ...
            text = f"<b>{label} – Cách {step}:</b> {count} cầu"
...
if __name__ == '__main__':
    ...
    # The command in railway.json or Procfile would be: gunicorn app:app
